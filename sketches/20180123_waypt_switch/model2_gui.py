...
waypoints can be temporarily inhibited to prevent it from interfering during
this process. Following a route is imitated by advancing the robot position,
which must be manually changed using Nengo GUI. Likewise, the temporary
inhibition of the cleanup memory for waypoints must be manually elicited using
Nengo GUI.
"""
...
with model:
    ...
    # Create node for inhibiting cleanup memory for waypoints
    waypt_cleanup_inh_inp = nengo.Node(0)
    nengo.Connection(waypt_cleanup_inh_inp, model.waypt_cleanup.inhibit,
                     transform=10.0)
